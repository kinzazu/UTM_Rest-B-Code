...
def create_db(db_name: str):
    ...
    try:
        cursor.execute('''CREATE TABLE alc_data (name text, my_code text, form_b text, ready integer, have_form int)''')
        connect.commit()
    except sqlite3.OperationalError:
        add_column(db_name)
...
def insert_data(db_name: str, ins_alc_class_list: list):    # Вставляет данные в таблицу SQL
    ...
    if len(result) == 6:
        ...
        print('DB_files_old is old. check if "HAVE_mark" COLUMN EXIST! \n attempt to add columns')
        try:
            cursor.execute('''ALTER TABLE alc_data ADD COLUMN have_form INTEGER''')
            connect.commit()
        except sqlite3.OperationalError as e:
...
<<<<<<< Updated upstream:DB_solver.py
        ...
        cursor.execute(sql)
=======
        sql = """SELECT form_b, my_code FROM alc_data WHERE ready == 0"""
>>>>>>> Stashed changes:DB/DB_solver.py
